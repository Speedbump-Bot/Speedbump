namespace Speedbump
{
    public static class XPConnector
    {
        public static void Increment(ulong guild, ulong user) =>
            new OldSqlInstance().Execute(@"insert into @p0xp values (@p1,@p2,1)
	                                        on duplicate key update xp=xp+1", guild, user);

        public static int GetXP(ulong guild, ulong user) =>
            (int)Math.Floor((decimal)new OldSqlInstance().Read(@"select coalesce(sum(xp), 0) from @p0xp where guild=@p1 and user=@p2", guild, user).Rows[0][0]);
        ...
        public static List<XPLevel> GetLevels(ulong guild) =>
            new OldSqlInstance().Read("select * from @p0level_role where guild=@p1", guild).Bind<XPLevel>();
        ...
        public static bool AddLevel(XPLevel level)
        {
            ...
            new OldSqlInstance().Execute("insert into @p0level_role values (@p1, @p2, @p3)", level.Guild, level.Level, level.Role);
            return true;
        }
        ...
        public static bool DeleteLevel(XPLevel level)
        {
            ...
            new OldSqlInstance().Execute("delete from @p0level_role where guild=@p1 and level = @p2", level.Guild, level.Level);
            return true;
        }
    }
}
